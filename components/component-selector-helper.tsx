...
export function ComponentSelectorHelper() {
  ...
  // Helper para verificar si un elemento es de texto
  ...
  // Helper para crear un hash de texto
  ...
  // Función para generar el pathHash de un elemento
  const generatePathHash = (element: HTMLElement): string => {
    const path: string[] = [];
    let currentElement: HTMLElement | null = element;
    // IMPORTANTE: Asegurar que llegamos lo más arriba posible
    ...
    // Aumentado a 5 niveles para mayor unicidad
    return path.slice(-5).join('-').replace(/[^a-zA-Z0-9-]/g, '-').toLowerCase();
  };

  // Función para extraer clase, path y texto de un componentId
  const parseComponentId = (componentId: string): { classPart: string; pathPart: string; textPart: string } => {
  ...
  useEffect(() => {
    ...
    if (!isInIframe) {
        ...
        if (cssContent) {
          ...
          componentIdsInCSS.forEach(componentId => {
            ...
            if (!element) {
              ...
              const { classPart, pathPart, textPart } = parseComponentId(componentId);
              ...
              if (classPart) {
                ...
                if (!foundElement) {
                  ...
                  if (candidates.length > 0) {
                    const { classPart, pathPart, textPart } = parseComponentId(componentId);
                    const normalizedPathFromId = pathPart.replace(/--+/g, '-').replace(/^-|-$/g, '').toLowerCase();

                    for (const candidate of candidates) {
                      const candidatePath = generatePathHash(candidate);
                      const normalizedCandidatePath = candidatePath.replace(/--+/g, '-').replace(/^-|-$/g, '');

                      // Verificar si el path coincide (exacto o contenido)
                      const isPathMatch = normalizedCandidatePath === normalizedPathFromId ||
                        normalizedPathFromId.includes(normalizedCandidatePath) ||
                        normalizedCandidatePath.includes(normalizedPathFromId);

                      if (isPathMatch) {
                        // Si hay parte de texto, verificar que coincida con el contenido del elemento
                        if (textPart) {
                          const candidateText = candidate.innerText?.toLowerCase() || '';
                          const candidateTextHash = createTextHash(candidateText).replace(/^-/, '');

                          // Si el hash del texto coincide, hemos encontrado el elemento correcto
                          // IMPORTANTE: En modo standalone, si hay texto, la coincidencia debe ser estricta para evitar colisiones
                          if (candidateTextHash.includes(textPart) || textPart.includes(candidateTextHash)) {
                            foundElement = candidate;
                            break;
                          }
                        } else {
                          // Si no hay texto en el ID, nos basamos solo en el path
                          foundElement = candidate;
                          break;
                        }
                      }
                    ...
                    // 🔥 NUEVO: Si no se encontró por path pero el ID tiene TEXTO, buscar CUALQUIER elemento con ese texto
                    if (!foundElement && textPart) {
                      console.log(`[component-selector-helper] 🔍 Buscando por texto estricto: "${textPart}"`);
                      for (const candidate of candidates) {
                        const candidateText = candidate.innerText?.toLowerCase() || '';
                        const candidateTextHash = createTextHash(candidateText).replace(/^-/, '');
                        if (candidateTextHash.includes(textPart) || textPart.includes(candidateTextHash)) {
                          foundElement = candidate;
                          console.log(`[component-selector-helper] 🎯 Encontrado por texto estricto:`, candidate.tagName);
                          break;
                        }
                      }
                    }
      ...
      const tryApplyIds = () => {
        applyIdsFromImportedCSSStandalone().catch(err => {
          ...
          // Despues de aplicar IDs, intentar aplicar iconos si zeus-icons.js esta cargado
          if (typeof window !== 'undefined' && (window as any).__zeusIcons) {
            console.log('[component-selector-helper] IDs aplicados, invocando window.__zeusIcons.applyIcons()');
            try {
              (window as any).__zeusIcons.applyIcons();
            } catch (e) {
              console.warn('[component-selector-helper] Error al invocar applyIcons:', e);
            }
          }
        });
      };

      // Retrasar la aplicacion de IDs para evitar hydration mismatch de React
      // Next.js hidrata despues de DOMContentLoaded; aplicar IDs antes causa mismatch
      const scheduleApplyIds = () => {
        // Primer intento rapido pero despues del primer paint
        setTimeout(tryApplyIds, 800);
        // Reintentos mas tardios por si React tarda en hidratar
        setTimeout(tryApplyIds, 1500);
        setTimeout(tryApplyIds, 2500);
        setTimeout(tryApplyIds, 4000);
      };

      if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', scheduleApplyIds);
      } else {
        scheduleApplyIds();
      }
      // Tambien reintentar despues de window.load por si acaso
    ...
    let pendingIconProperties: Record<string, any> | null = null;
    ...
    function parseComponentId(componentId: string): { classPart: string; pathPart: string; textPart: string } {
      const idWithoutComponent = componentId.replace(/^component-/, '');
    ...
    function getIconPath(iconName: string): string {
      ...
      // Fallback al mapa generado por zeus-icons.js si está disponible
      const zeusPaths = (typeof window !== 'undefined' && (window as any).__zeusIcons?.iconPaths) || {};
      // Si tenemos el path del icono, usarlo; si no, usar un icono genérico (círculo)
      return iconPaths[iconName] || zeusPaths[iconName] || 'M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8z';
    }
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (event.data && event.data.type === 'enableSelection') {
        ...
        // Build CSS-based icons instead of DOM injection (React-safe)
        let iconCss = '';
        Object.entries(iconProperties).forEach(([componentId, props]: [string, any]) => {
          if (!props || !props.icon || !props.icon.name) return;
          ...
          // Hide any old injected SVGs/containers
          iconCss += `[data-component-id="${componentId}"] .zeus-injected-icon,\n`;
          iconCss += `[data-component-id="${componentId}"] > svg[viewBox="0 0 24 24"],\n`;
          iconCss += `[data-component-id="${componentId}"] svg.lucide,\n`;
          iconCss += `[data-component-id="${componentId}"] .lucide {\n`;
          iconCss += `  display: none !important;\n`;
          iconCss += `}\n\n`;
          ...
          iconCss += `[data-component-id="${componentId}"]::before {\n`;
          iconCss += `  content: '' !important;\n`;
          iconCss += `  display: inline-block !important;\n`;
          iconCss += `  width: ${iconSize}px !important;\n`;
          iconCss += `  height: ${iconSize}px !important;\n`;
          iconCss += `  background-image: ${dataUri} !important;\n`;
          iconCss += `  background-size: contain !important;\n`;
          iconCss += `  background-repeat: no-repeat !important;\n`;
          iconCss += `  background-position: center !important;\n`;
          iconCss += `  vertical-align: middle !important;\n`;
          iconCss += `  flex-shrink: 0 !important;\n`;
          iconCss += `}\n\n`;
        });

        if (iconCss) {
          // Remove old live icon styles
          const existing = document.getElementById('zeus-live-icons');
          if (existing) existing.remove();

          const style = document.createElement('style');
          style.id = 'zeus-live-icons';
        ...
        elementsToProcess.forEach((element) => {
          ...
          for (const existingId of existingComponentIds) {
            ...
            const { classPart, pathPart, textPart } = parseComponentId(existingId);
